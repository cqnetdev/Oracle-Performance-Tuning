...
from (select * from SOLAR_diff_stats where ELAPSED_TIME > 0 order by ELAPSED_TIME desc) where rownum <=50;
...
from (select * from SOLAR_diff_stats where DISK_READS > 0 order by DISK_READS desc), global_name where rownum <=50;
