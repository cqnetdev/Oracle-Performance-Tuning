...
select
  ...
  -- ,dbtime.max_db_time
from
  (select i.instance_number, snap_id, sum(CPU_TIME_DELTA)/1000000 as CPU, sum(IOWAIT_DELTA)/1000000 as IO,
     greatest(max(sum(CPU_TIME_DELTA/1000000)) over (), max(sum(IOWAIT_DELTA/1000000)) over () ) as limit
     from DBA_HIST_SQLSTAT, gV$instance i where i.instance_number=DBA_HIST_SQLSTAT.instance_number and CPU_TIME_DELTA >= 0 and IOWAIT_DELTA >= 0 group by i.instance_number, snap_id /* order by snap_id */ ) load,
  dba_hist_snapshot snap, gV$instance i, DBTIME
where load.instance_number=i.instance_number and snap.snap_id=load.snap_id and i.instance_number=snap.instance_number and dbtime.SNAP_ID=snap.snap_id and dbtime.INSTANCE_NUMBER=snap.instance_number
