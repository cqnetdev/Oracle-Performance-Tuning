...
begin
  begin
    execute immediate  'drop table v$sql_plan_sel';
  exception
    when others then null;
  end;
  execute immediate  'create table v$sql_plan_sel as select * from v$sql_plan where sql_id=''&SQL_ID''';
end;
/
...
--  execute immediate  'create global temporary table v$sql_plan_sel as select * from v$sql_plan where sql_id=''&SQL_ID''';
  ...
  for c_plan in (SELECT OBJECT_ALIAS, level, object_name, sql_id, decode(optimizer,NULL,NULL,' (mode='||optimizer||')')  as optmode, operation, id, parent_id, cardinality, cost, CHILD_ADDRESS, plan_hash_value, object_owner, options, timestamp, access_predicates, filter_predicates  from v$sql_plan_sel CONNECT BY prior id = parent_id  AND prior sql_id = sql_id and prior CHILD_ADDRESS = CHILD_ADDRESS START WITH id = 0 AND CHILD_ADDRESS=l_child AND sql_id = '&SQL_ID' ORDER BY CHILD_ADDRESS, id) loop
    ...
    l_next := l_space;
    select count(*) into l_more from v$sql_plan_sel where sql_id = c_plan.sql_id and CHILD_ADDRESS = c_plan.CHILD_ADDRESS and parent_id=c_plan.parent_id and id>c_plan.id;
    ...
    select count(*) into l_children from v$sql_plan_sel where sql_id = c_plan.sql_id and CHILD_ADDRESS = c_plan.CHILD_ADDRESS and parent_id=c_plan.id;
    if c_plan.id-c_plan.parent_id>1 then
      select ' ->'||lower(operation)||':'||id into l_parent from v$sql_plan_sel where sql_id = c_plan.sql_id and CHILD_ADDRESS = c_plan.CHILD_ADDRESS and id=c_plan.parent_id;
    else
      l_parent := NULL;
    end if;
  ...
  dbms_output.put_line(lpad('-',110,'-'));
  for c_plan2 in (SELECT * from v$sql_plan_sel where CHILD_ADDRESS=l_child AND sql_id = '&SQL_ID' ORDER BY CHILD_ADDRESS, id) loop
    if c_plan2.ACCESS_PREDICATES is not null then 
      dbms_output.put_line(rpad(to_char(c_plan2.id),3)||'| Access: '||c_plan2.ACCESS_PREDICATES);
    end if;
    if c_plan2.FILTER_PREDICATES is not null then 
      dbms_output.put_line(rpad(to_char(c_plan2.id),3)||'| Filter: '||c_plan2.FILTER_PREDICATES);
    end if;
  end loop;
end;
/
