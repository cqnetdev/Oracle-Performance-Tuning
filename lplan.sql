...
begin
  ...
  for c_plan in (SELECT level, object_name, sql_id, decode(optimizer,NULL,NULL,' (mode='||optimizer||')')  as optmode, operation, id, parent_id, cardinality, cost, CHILD_ADDRESS, plan_hash_value, object_owner, options, timestamp, access_predicates, filter_predicates  from v$sql_plan CONNECT BY prior id = parent_id  AND prior sql_id = sql_id and prior CHILD_ADDRESS = CHILD_ADDRESS START WITH id = 0 AND CHILD_ADDRESS=l_child AND sql_id = '&SQL_ID' ORDER BY CHILD_ADDRESS, id) loop
    ...
    dbms_output.put_line(rpad(c_plan.id,3)||'| '||rpad(l_prefix||l_next||c_plan.operation||' '||
      c_plan.options||' '||c_plan.object_name||l_parent||c_plan.optmode,82)||'|'||lpad(nvl(to_char(c_plan.cardinality),' '),10)||'|'||lpad(nvl(to_char(c_plan.cost),' '),10)||'|');
    if l_more=0 then
